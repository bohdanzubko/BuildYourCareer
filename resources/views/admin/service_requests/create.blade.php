...
@section('content')
<div class="container">
    <h1 class="page-title mb-4">Create Service Request</h1>
    ...
    <form action="{{ route('service-requests.store') }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="number" step="0.01" name="area" id="area" class="form-control" value="{{ old('area') }}">
        </div>

        <div class="mb-3">
            <label for="estimated_price" class="form-label">Estimated Price</label>
            <input type="number" step="0.01" name="estimated_price" id="estimated_price" class="form-control" value="{{ old('estimated_price') }}">
        </div>
