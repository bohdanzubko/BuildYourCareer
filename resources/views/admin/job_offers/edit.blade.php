...
@section('content')
<div class="container">
    <h1 class=" page-title mb-4">Edit Job Offer</h1>
    ...
    <form action="{{ route('job_offers.update', $jobOffer->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select class="form-control @error('job_id') is-invalid @enderror" id="job_id" name="job_id" required>
                ...
                @foreach($jobs as $job)
                    <option value="{{ $job->id }}" {{ old('job_id', $jobOffer->job_id) == $job->id ? 'selected' : '' }}>{{ $job->title }}</option>
                @endforeach
            </select>
            @error('job_id')
        ...
        <div class="mb-3">
            ...
            <select class="form-control @error('user_id') is-invalid @enderror" id="user_id" name="user_id" required>
                ...
                @foreach($users as $user)
                    <option value="{{ $user->id }}" {{ old('user_id', $jobOffer->user_id) == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                @endforeach
            </select>
            @error('user_id')
                <div class="invalid-feedback">{{ $message }}</div>
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('offer_price') is-invalid @enderror" id="offer_price" name="offer_price" value="{{ old('offer_price', $jobOffer->offer_price) }}" required>
            @error('offer_price')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
