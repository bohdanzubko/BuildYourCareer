...
<div class="container mt-4">
    <h1 class="mb-4 text-2xl font-semibold text-gray-200">Редагувати вакансію</h1>

    @include('profile.partials.menu')

    <form action="{{ route('profile.jobs.update', $job->id) }}" method="POST" enctype="multipart/form-data" class="bg-[#23272f] rounded shadow p-6 max-w-lg mx-auto">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <textarea name="description" id="description"
                      ...
                      rows="4" required>{{ old('description', $job->description) }}</textarea>
            @error('description')
                <div class="invalid-feedback text-red-400">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-4 flex gap-2">
            <div class="flex-1">
                ...
                <input type="number" name="salary_min" id="salary_min"
                       ...
                       min="0" step="0.01" value="{{ old('salary_min', $job->salary_min) }}" required>
                @error('salary_min')
                    <div class="invalid-feedback text-red-400">{{ $message }}</div>
                @enderror
            </div>
            <div class="flex-1">
                ...
                <input type="number" name="salary_max" id="salary_max"
                       ...
                       min="0" step="0.01" value="{{ old('salary_max', $job->salary_max) }}" required>
                @error('salary_max')
                    <div class="invalid-feedback text-red-400">{{ $message }}</div>
                @enderror
            </div>
            ...
            <select name="skills[]" id="skills" multiple
                ...
                @foreach($allSkills as $skill)
                    <option value="{{ $skill->id }}" 
                        @if(collect(old('skills', $jobSkills ?? []))->contains($skill->id)) selected @endif>
                        {{ $skill->name }}
                    </option>
                @endforeach
            </select>
