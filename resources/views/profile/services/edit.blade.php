...
<div class="container mt-4">
    <h1 class="mb-4 text-2xl font-semibold text-gray-200">Редагування послуги</h1>

    @include('profile.partials.menu')

    <form action="{{ route('profile.services.update', $service->id) }}" method="POST" enctype="multipart/form-data" class="bg-[#23272f] rounded shadow p-6 max-w-lg mx-auto">
        @csrf
        @method('PUT')
            ...
            <select name="skills[]" id="skills" multiple
                ...
                @foreach($skills as $skill)
                    <option value="{{ $skill->id }}" 
                        @if(collect(old('skills', $service->skills->pluck('id')->toArray() ?? []))->contains($skill->id)) selected @endif>
                        {{ $skill->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <small class="text-gray-400">Введіть ключові слова, розділені комою</small>
            @error('tags')
                <div class="invalid-feedback text-red-400">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            @if($service->file_path)
                <div class="mt-2">
                    <a href="{{ asset('storage/'.$service->file_path) }}" target="_blank" class="text-cyan-400 hover:underline">Поточний файл</a>
                </div>
            @endif
        </div>

        <div class="flex justify-end">
            <button type="submit" class="btn btn-primary action-btn">Зберегти зміни</button>
        </div>
    </form>
</div>
@endsection
